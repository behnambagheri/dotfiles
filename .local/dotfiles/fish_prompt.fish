...
#   set -l current_user bea
  ...
  if test (uname -n | cut -d . -f 1) = "Bagheri-MacBook-Pro"
      ...
      ## Line 1
      ##
      echo -n $hostcolor'╭─'$hotpink HOST: $current_user$white' at '$orange$__fish_prompt_hostname$white' in '$limegreen(pwd | sed "s=$HOME=⌁=")$turquoise $white'('$exit_color$last_status$white')'
